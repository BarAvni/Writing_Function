...
monthly_runoff is modelled runoff (organized by column) in order to make it easy, I'll set my data names and arrangment to class variables and datasets:

modQ is modelled runoff (organized by column)
obsQ is observed runoff
...
stats$nselog = tmp
# how about annual flows (wy = water year)
...
#stats$accuracy=pmax(0.0,stats$nse)*pmax(0.0,(1-abs(stats$mperr/100)))

# pmax(0.0,stats$nse) is zero (all original bellow zero), pmax(0.0, stats$nselog) is all infinity.
...
```
Now we might want to see how parameter uncertainty impacts other estimates - For example February and June flow
...
# lets look at February (snow interception model)
...
#boxplot(t(modQ.feb[,1:nsims]), ylab="February Flow", names=modQ.feb$wy)
...
#boxplot(t(modQ.feb[,accept.cols]), ylab="February Flow", names=modQ.feb$wy)
...
# what about uncertainty in mean February flow (overall whole simulation period)
...
# lets look at how monthly streamflow varies for each year across the range across all parameter
boxplot(t(modQ.jun[,1:nsims]), ylab="June Flow", names=modQ.jun$wy)

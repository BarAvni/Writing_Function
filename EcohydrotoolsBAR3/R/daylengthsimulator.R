#' this function simulates a day length in a specified location
#' the user is asked to provide a minimum and maximum day length 
#' the values are arranged from JAN 1st to DEC 31st
#'
...
#' @author Yoav BD
#' @return daily hours for one year (arbitrary units)
...
daylengthsimulator=
  function(mindaylength=10, maxdaylength=14,hemisphere="N"){
    ...
    for (ii in 1:length(days)) {
      ...
      if (days[ii]>365) days[ii]=days[ii]-maxdays
    }
